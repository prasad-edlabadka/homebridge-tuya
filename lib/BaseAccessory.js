class BaseAccessory {
    constructor(...props) {
        ...
        this.device.context.name = this._sanitizeHomeKitName(this.device.context.name);
        ...
        this.device.once('connect', () => {
            ...
            if (typeof this._onDeviceConnect === 'function') this._onDeviceConnect();
        });
    ...
    _checkServiceName(service, name) {
        ...
        const sanitizedName = this._sanitizeHomeKitName(name);

        if (service.displayName !== sanitizedName) {
            const nameCharacteristic = service.getCharacteristic(Characteristic.Name) || service.addCharacteristic(Characteristic.Name);
            nameCharacteristic.setValue(sanitizedName);
            service.displayName = sanitizedName;
        }
    }

    _sanitizeHomeKitName(name, fallback = 'Unnamed') {
    ...
    setMultiStateLegacy(dps, callback) {
        ...
        if (!this.device.connected) return callback && callback(true);
    ...
    setMultiState(dps, callback) {
        ...
        for (const dp in dps) {
            if (dps.hasOwnProperty(dp) && dps[dp] !== this.device.state[dp]){
                ret = this.device.update({[dp.toString()] : dps[dp]}) && ret;
            }
        }
        callback && callback(!ret);
    }
