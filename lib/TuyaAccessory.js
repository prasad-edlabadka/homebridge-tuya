...
class TuyaAccessory extends EventEmitter {
    ...
    _connect() {
        ...
        (this._socket.reconnect = () => {
            this.log.debug(`Connect attempt for ${this.context.name} (${this.context.version}) at ${this.context.ip}:${this.context.port}.`);
        ...
        this._socket.on('connect', () => {
            if (this.context.version !== '3.4') {
                ...
                this.connected = true;
                this.log.debug(`Socket connected to ${this.context.name} (${this.context.version}).`);
                this.emit('connect');
                if (this._socket._pinger)
                    clearTimeout(this._socket._pinger);
        ...
        this._socket.on('error', err => {
            ...
            this.log.debug(`Connect path failed for ${this.context.name} (${this.context.version}) at ${this.context.ip}:${this.context.port}: ${err && (err.stack || err.message || err)}`);
    ...
    update(o) {
        ...
        if (hasDataPoint) {
            ...
            this.log.debug(`Sending first state query to ${this.context.name} (${this.context.version}).`);
            result = this._send({
                data: {
    ...
    _change(data) {
        ...
        if (isNonEmptyPlainObject(changes)) {
            ...
            this.log.debug(`State update from ${this.context.name}: ${JSON.stringify(changes)}`);
